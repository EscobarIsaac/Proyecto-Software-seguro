#!/usr/bin/env python3
"""
Pruebas unitarias para el sistema de detección de vulnerabilidades
...
class TestVulnerabilityModel:
    """Pruebas del modelo de detección de vulnerabilidades"""
    ...
    def test_model_accuracy_requirement(self, model_and_data):
        """
        TEST CRÍTICO: Validar que el modelo cumple con accuracy mínimo de 82%
        Requisito del documento: "Accuracy mínima demostrada: 82%"
        """
        ...
        print(f"\n📊 Accuracy con validación cruzada 5-fold: {mean_accuracy:.4f}")
        print(f"   Scores individuales: {cv_scores}")
        print(f"   Desviación estándar: {cv_scores.std():.4f}")
        
        # VALIDACIÓN CRÍTICA: Debe ser >= 82%
        assert mean_accuracy >= 0.82, f"Accuracy {mean_accuracy:.2%} es menor al 82% requerido"
        print(f"   ✅ CUMPLE: Accuracy {mean_accuracy:.2%} >= 82%")
    ...
    def test_model_training_accuracy(self, model_and_data):
        ...
        print(f"\n📊 Accuracy en datos de entrenamiento: {train_accuracy:.4f}")
        
        # El accuracy en training debería ser alto
    ...
    def test_model_prediction_format(self, model_and_data):
        ...
        # Validar formato
        assert len(predictions) == 5, "Número de predicciones incorrecto"
        ...
        print("\n✅ Formato de predicciones correcto")
    
    def test_feature_extraction(self):
        """Validar extracción de características"""
        ...
        # Validar que se extraen 12 características (sin score)
        assert len(features_vulnerable) == 12, "Debe extraer 12 características"
        assert len(features_safe) == 12, "Debe extraer 12 características"
        
        # El código vulnerable debe tener mayor sql_risk
        assert features_vulnerable.iloc[7] > features_safe.iloc[7], "SQL risk no detectado"
        
        print("\n✅ Extracción de características funciona correctamente")
    ...
    def test_alert_levels(self, model_and_data):
        ...
        print(f"   🚨 Críticas (>70%): {critical}")
        print(f"   ⚠️  Medias (50-70%): {medium}")
        print(f"   ✅ Bajas (<50%): {low}")
        ...
        print("   ✅ Sistema de alertas funciona correctamente")


class TestFeatureExtraction:
    """Pruebas de extracción de características"""
    ...
    def test_sql_injection_detection(self):
        ...
        # Índice 7 es sql_risk
        assert features.iloc[7] > 0, "No detectó patrones SQL"
        print("\n✅ Detección de SQL injection funciona")
    ...
    def test_xss_detection(self):
        ...
        # Índice 8 es xss_risk
        assert features.iloc[8] > 0, "No detectó patrones XSS"
        print("\n✅ Detección de XSS funciona")
    ...
    def test_dangerous_functions_detection(self):
        ...
        # Índice 10 es dangerous_count
        assert features.iloc[10] > 0, "No detectó funciones peligrosas"
        print("\n✅ Detección de funciones peligrosas funciona")
...
class TestAPIEndpoints:
    ...
    def test_analyze_endpoint_vulnerable(self, client):
        """Probar análisis de código vulnerable"""
        vulnerable_code = "query = 'SELECT * FROM users WHERE id = ' + user_input"
        ...
        print(f"\n✅ Análisis de código vulnerable: {data['alert_level']}")
    
    def test_analyze_endpoint_safe(self, client):
        """Probar análisis de código seguro"""
        safe_code = "x = 5 + 3\nprint(x)"
        ...
        print(f"\n✅ Análisis de código seguro: {data['alert_level']}")
...
class TestPipelineIntegration:
    ...
    def test_dataset_format(self):
        ...
        # Debe tener 13 características + 1 label
        ...
        print(f"\n✅ Dataset tiene formato correcto: {train_df.shape[0]} muestras, {train_df.shape[1]} columnas")
